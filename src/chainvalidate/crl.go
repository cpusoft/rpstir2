...
func getChainCrls(chains *Chains, chainWg *sync.WaitGroup, syncLogId uint64) {
	...
	belogs.Debug("getChainCrls(): after GetChainCrlData before roaWg.Wait(), time(s):", time.Since(start))
	crlWg.Wait()
	close(chainCrlDatasCh)
	belogs.Info("getChainCrls(): end, syncLogId:", syncLogId, " time(s):", time.Since(start))
}

func callAddCrlToChain(chains *Chains, syncLogId uint64, chainCrlDatasCh chan []*ChainCertData, crlWg *sync.WaitGroup) {
	start := time.Now()
	var index uint64
	for {
		select {
		case chainCrlDatas, ok := <-chainCrlDatasCh:
			belogs.Debug("callAddCrlToChain(): get from chainCrlDatasCh, len(chainCrlDatas):", len(chainCrlDatas),
				"  index:", index, "  ok:", ok)
			if ok {
				go addCrlToChain(chains, chainCrlDatas, syncLogId, crlWg)
				index++
				belogs.Debug("callAddCrlToChain(): addCrlToChain index:", index)
			} else {
				belogs.Info("callAddCrlToChain(): close chainCrlSqlsCh, index:", index, "   time(s):", time.Since(start))
				return
			}
		}
	}
...
func addCrlToChain(chains *Chains, chainCrlDatas []*ChainCertData, syncLogId uint64, crlWg *sync.WaitGroup) {
	defer func() {
		crlWg.Done()
		belogs.Debug("addCrlToChain(): crlWg.Done(), len(chainCrlDatas):", len(chainCrlDatas))
	}()

	start := time.Now()
	for i := range chainCrlDatas {
		startOne := time.Now()
		belogs.Debug("addCrlToChain(): chainCrlDatas:", jsonutil.MarshalJson(chainCrlDatas[i]), "  syncLogId:", syncLogId)

		chainCrl, err := chainCrlDatas[i].ToChainCrl()
		if err != nil {
		...
		belogs.Debug("addCrlToChain(): added, chainCrl:", jsonutil.MarshalJson(chainCrl),
			"  syncLogId:", syncLogId, "    chainCrlSql.SyncLogId:", chainCrlDatas[i].SyncLogId,
			"  time(s):", time.Since(startOne))
	}
	belogs.Info("addCrlToChain(): all added, len(chainCrlDatas):", len(chainCrlDatas), "  time(s):", time.Since(start))
	return
}
...
func validateCrls(chains *Chains, wg *sync.WaitGroup) {
	...
	var crlWg sync.WaitGroup
	chainCrlCh := make(chan int, conf.Int("chain::validateConcurrentCount"))
	...
	belogs.Info("validateCrls(): validate end, len(crlIds):", len(crlIds), "  time(s):", time.Since(start))
}
...
func validateCrl(chains *Chains, crlId uint64, wg *sync.WaitGroup, chainCrlCh chan int) {
	...
	belogs.Debug("validateCrl(): after check ParentChainCerAlones, crlId:", crlId, "  time(s):", time.Since(start))
	...
	belogs.Debug("validateCrl(): after check sameAkiChainCrls, crlId:", crlId, "  time(s):", time.Since(start))
	...
	chains.UpdateFileTypeIdToCrl(&chainCrl)
	belogs.Info("validateCrl(): validate one end, UpdateFileTypeIdToCrl, crlId:", crlId,
		" crl file:", chainCrl.FilePath, chainCrl.FileName, "  time(s):", time.Since(start))
}
...
func getCrlParentChainCers(chains *Chains, crlId uint64) (chainCerAlones []ChainCerAlone, err error) {
	...
	belogs.Debug("getCrlParentChainCers(): crlId:", crlId, "  parentChainCerAlone.Id:", parentChainCerAlone.Id, "   time(s):", time.Since(start))
	...
	belogs.Debug("getCrlParentChainCers(): end, crlId:", crlId, "   len(chainCerAlones):", len(chainCerAlones), "  time(s):", time.Since(start))
	return chainCerAlones, nil
}
...
func getSameAkiChainCrls(chains *Chains, crlId uint64) (sameAkiChainCrls []ChainCrl, err error) {
	...
	belogs.Debug("getSameAkiChainCrls(): AkiToFileTypeIds, aki:", aki, "  crlId:", crlId, "  ok:", ok)
	if ok {
		belogs.Debug("getSameAkiChainCrls(): ok AkiToFileTypeIds, crlId:", crlId, "  fileTypeIds:", fileTypeIds)
	...
	belogs.Debug("getSameAkiChainCrls(): end, crlId:", crlId, "  time(s):", time.Since(start))
	return
}
...
func updateCrls(chains *Chains, wg *sync.WaitGroup) {
	...
	belogs.Info("updateCrls(): ok,  time(s):", time.Since(start))
}
