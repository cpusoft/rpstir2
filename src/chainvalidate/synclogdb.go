...
func updateRsyncLogChainValidateStateStartDb(state string) (labRpkiSyncLogId uint64, err error) {
	...
	if err != nil {
		return 0, xormdb.RollbackAndLogError(session, "updateRsyncLogChainValidateStateStartDb(): update sync_log fail: state:"+state, err)
	}
	...
	if err != nil {
		return 0, xormdb.RollbackAndLogError(session, "updateRsyncLogChainValidateStateStartDb(): UPDATE sync_log fail: chainValidateState:"+
