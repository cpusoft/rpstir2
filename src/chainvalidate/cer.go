...
func getChainCers(chains *Chains, chainWg *sync.WaitGroup, syncLogId uint64) {
	...
	belogs.Debug("getChainCers(): after GetChainCerData before cerWg.Wait(), time(s):", time.Since(start))
	cerWg.Wait()
	close(chainCerDatasCh)
	belogs.Debug("getChainCers(): end, syncLogId:", syncLogId, " time(s):", time.Since(start))
}

func callAddCerToChain(chains *Chains, syncLogId uint64, chainCerDatasCh chan []*ChainCertData, cerWg *sync.WaitGroup) {
	start := time.Now()
	var index uint64
	for {
		select {
		case chainCerDatas, ok := <-chainCerDatasCh:
			belogs.Debug("callAddCerToChain(): get from chainCerDatasCh, len(chainCerDatas):", len(chainCerDatas),
				"  index:", index, "  ok:", ok)
			if ok {
				go addCerToChain(chains, chainCerDatas, syncLogId, cerWg)
				index++
				belogs.Debug("callAddCerToChain(): addCerToChain index:", index)
			} else {
				belogs.Info("callAddCerToChain(): close chainCerSqlsCh, index:", index, "   time(s):", time.Since(start))
				return
			}
		}
	}
...
func addCerToChain(chains *Chains, chainCerDatas []*ChainCertData, syncLogId uint64, cerWg *sync.WaitGroup) {
	defer func() {
		cerWg.Done()
		belogs.Debug("addCerToChain(): cerWg.Done(), len(chainCerDatas):", len(chainCerDatas))
	}()

	start := time.Now()
	for i := range chainCerDatas {
		startOne := time.Now()
		belogs.Debug("addCerToChain(): chainCerDatas:", jsonutil.MarshalJson(chainCerDatas[i]), "  syncLogId:", syncLogId)
		chainCer, err := chainCerDatas[i].ToChainCer()
		if err != nil {
		...
		belogs.Debug("addCerToChain(): added, chainCer:", jsonutil.MarshalJson(chainCer),
			"  syncLogId:", syncLogId, "    chainCerDatas.SyncLogId:", chainCerDatas[i].SyncLogId,
			"  time(s):", time.Since(startOne))
	}
	belogs.Info("addCerToChain(): all added, len(chainCerDatas):", len(chainCerDatas), "  time(s):", time.Since(start))
	return
}
...
func validateCers(chains *Chains, wg *sync.WaitGroup) {
	...
	var cerWg sync.WaitGroup
	chainCerCh := make(chan int, conf.Int("chain::validateConcurrentCount"))
	...
	belogs.Info("validateCers(): validate end, len(cerIds):", len(cerIds), "  time(s):", time.Since(start))
}
...
func validateCer(chains *Chains, cerId uint64, wg *sync.WaitGroup, chainCerCh chan int) {
	...
	belogs.Debug("validateCer():GetCerParentChainCers, cerId:", cerId, "  len(chainCer.ParentChainCers):", len(chainCer.ParentChainCerAlones),
	...
	belogs.Debug("validateCer(): after check root or ParentChainCerAlones, cerId:", cerId, "  file:", chainCer.FilePath, chainCer.FileName,
		"  time(s):", time.Since(start))
	...
	chains.UpdateFileTypeIdToCer(&chainCer)
	belogs.Info("validateCer(): validate one end, UpdateFileTypeIdToCer, cerId:", cerId,
		"  cer file:", chainCer.FilePath, chainCer.FileName, "  time(s):", time.Since(start))
	return
...
func GetCerParentChainCers(chains *Chains, cerId uint64) ([]ChainCerAlone, error) {
	...
	if chainCer.IsRoot {
		belogs.Debug("GetCerParentChainCers(): cerId IsRoot, cerId:", cerId, "  chainCer.IsRoot:", chainCer.IsRoot)
	...
	for {
		...
		belogs.Debug("GetCerParentChainCers(): getCerParentChainCer, cerId:", cerId, "   parentChainCer.Id:", parentChainCer.Id, err)
		if err != nil {
		...
		if parentChainCer.IsRoot {
			belogs.Debug("GetCerParentChainCers(): found IsRoot, cerId:", cerId, "   parentChainCer.Id:", parentChainCer.Id)
			return chainCerAlones, nil
		}
...
func getCerParentChainCer(chains *Chains, cerId uint64) (parentChainCer ChainCer, err error) {
	...
	belogs.Debug("getCerParentChainCer(): found in SkiToFileTypeId, cerId:", cerId,
		"  parentCerSki:", parentCerSki, "  fileTypeId:", fileTypeId, "  ok:", ok)
	if ok {
		parentChainCer, err = chains.GetCerByFileTypeId(fileTypeId)
		belogs.Debug("getCerParentChainCer(): GetCerByFileTypeId, cerId:", cerId, " fileTypeId:", fileTypeId, " parentChainCer.Id:", parentChainCer.Id,
			"  time(s):", time.Since(start))
		if err != nil {
			belogs.Error("getCerParentChainCer(): GetCerByFileTypeId fail, cerId:", cerId, "  fileTypeId:", fileTypeId, err)
			return parentChainCer, err
		}
		return parentChainCer, nil
	}
	//  not found parent ,is not error
	belogs.Debug("getCerParentChainCer(): not found cer's parent cer:", cerId, "  time(s):", time.Since(start))
	return parentChainCer, nil
}
...
func updateCers(chains *Chains, wg *sync.WaitGroup) {
	...
	belogs.Info("updateCers(): ok,  time(s):", time.Since(start))
}
