...
func ParseMftModelByOpensslResults(results []string, mftModel *model.MftModel) (err error) {
	...
	belogs.Debug("ParseMftModelByOpensslResults(): get mft from mftByte, update time:", mft.ThisUpdate, mft.NextUpdate,
		"   len(mft.FileList):", len(mft.FileList))
	...
	if len(mft.FileList) > 0 {
		...
		belogs.Debug("ParseMftModelByOpensslResults(): len(mft.FileList):", len(mft.FileList))
		...
		for _, one := range mft.FileList {
			...
			belogs.Debug("ParseMftModelByOpensslResults(): range mft.FileList, fileHashModel:", jsonutil.MarshalJson(fileHashModel))
		}
		mftModel.FileHashModels = fileHashModels
		belogs.Debug("ParseMftModelByOpensslResults(): mftModel:", mftModel.String())
		return nil
	}
	// using raw try again
	...
	mftModel.FileHashModels = fileHashModels
	belogs.Debug("ParseMftModelByOpensslResults():raw get mftModel:", mftModel.String())
	return nil
}
...
func ParseRoaModelByOpensslResults(results []string, roaModel *model.RoaModel) (err error) {
	...
	*/
	...
	belogs.Debug("ParseRoaModelByOpensslResults(): roaModel:", roaModel.String())
...
func ParseMftEContentTypeByOpensslResults(results []string) (eContentType string, err error) {
	...
	oid := "1.2.840.113549.1.9.16.1.26" // openssl 1.**
	oidStr := "id-ct-rpkiManifest"      // openssl 3.**
	for _, one := range results {
		if strings.Contains(one, oid) || strings.Contains(one, oidStr) {
			return oid, nil
		}
	}
	return "", errors.New("invalid mft EEContent type")
}
func ParseRoaEContentTypeByOpensslResults(results []string) (eContentType string, err error) {
	// get 1.2.840.113549.1.9.16.1.24
	oid := "1.2.840.113549.1.9.16.1.24" // openssl 1.**
	oidStr := "id-ct-routeOriginAuthz"  // openssl 3.**
	for _, one := range results {
		if strings.Contains(one, oid) || strings.Contains(one, oidStr) {
			return oid, nil
		}
	}
	return "", errors.New("invalid ROA EEContent type")
}
...
func ParseByOpensslAns1ToX509(certFile string, results []string) (cerFile *os.File, fileByte []byte,
	...
	belogs.Debug("ParseByOpensslAns1ToX509(): start certFile:", certFile, "   len(results):", len(results))
	// get cer info
	...
	*/
	...
	cerEndStr2 := "prim: EOC"
	for i := range results {
		belogs.Debug("ParseByOpensslAns1ToX509(): rang results, certFile:", certFile, "  i:", i, "   len(results[i]):", len(results[i]))
	...
	belogs.Debug("ParseByOpensslAns1ToX509(): after results, certFile:", certFile, "  cerStartIndex:", cerStartIndex, "   cerEndIndex:", cerEndIndex)
	...
	_, fileDecodeBase64Byte, err := fileutil.ReadFileAndDecodeCertBase64(certFile)
	if err != nil {
...
func ParseSigEContentTypeByOpensslResults(results []string) (eContentType string, err error) {
	...
	*/
	...
	return "", errors.New("invalid SIG EEContent type")
}
...
func ParseAsaModelByOpensslResults(results []string, asaModel *model.AsaModel) (err error) {
	...
	*/
	...
	if err != nil {
		belogs.Debug("ParseAsaModelByOpensslResults(): use AsProviderAttestation, asn1.Unmarshal fail:", err)

		// try use old rfc

		asProviderAttestationOld := AsProviderAttestationOld{}
		_, err = asn1.Unmarshal(asaBytes, &asProviderAttestationOld)
		if err != nil {
			belogs.Error("ParseAsaModelByOpensslResults():use AsProviderAttestationOld, asn1.Unmarshal fail:", err)
			return err
		}
		belogs.Debug("ParseAsaModelByOpensslResults():asProviderAttestationOld:", jsonutil.MarshalJson(asProviderAttestationOld))
		asProviderAttestation = convertAsProviderAttestationOldToAsProviderAttestation(asProviderAttestationOld)
	}
	belogs.Debug("ParseAsaModelByOpensslResults():asProviderAttestation:", jsonutil.MarshalJson(asProviderAttestation))
...
func ParseAsaEContentTypeByOpensslResults(results []string) (eContentType string, err error) {
	...
	*/
	oid := "1.2.840.113549.1.9.16.1.49" // openssl 1.**
	oidStr := "id-ct-ASPA"              // openssl 3.**
	for _, one := range results {
		if strings.Contains(one, oid) || strings.Contains(one, oidStr) {
			return oid, nil
		}
	}
	return "", errors.New("invalid ASA EEContent type")
}
