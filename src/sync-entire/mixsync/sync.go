...
import (
	...
	coremodel "github.com/bgpsecurity/rpstir2/sync-core/model"
	"github.com/bgpsecurity/rpstir2/sync-core/rrdp"
	coresync "github.com/bgpsecurity/rpstir2/sync-core/sync"
...
func getTals() (talModels []model.TalModel, err error) {
	...
	err = httpclient.PostAndUnmarshalResponseModelWithConfig("https://"+conf.String("rpstir2-rp::serverHost")+":"+conf.String("rpstir2-rp::serverHttpsPort")+
		"/tal/gettals", "", &talModelsResponse,
...
func callSync(syncLogId uint64, talModels []model.TalModel, syncState *coremodel.SyncState) (err error) {
	...
	destPathRoot := conf.String("rrdp::destPath") + "/root/"
	os.RemoveAll(destPathRoot)
	os.MkdirAll(destPathRoot, os.ModePerm)
	belogs.Debug("callSync(): removeAll and mkdirAll, destPathRoot:", destPathRoot,
		"  len(talModels):", len(talModels))
	for _, talModel := range talModels {
		for _, talSyncUrl := range talModel.TalSyncUrls {
			...
			if talSyncUrl.SupportRrdp && len(talSyncUrl.RrdpUrl) > 0 {
				...
				belogs.Debug("callSync(): support Rrdp, talSyncUrl:", jsonutil.MarshalJson(talSyncUrl), "  url:", url)
			} else {
				if talSyncUrl.SupportRsync && len(talSyncUrl.RsyncUrl) > 0 {
					url = talSyncUrl.RsyncUrl
					belogs.Debug("callSync(): support Rsync, talSyncUrl:", jsonutil.MarshalJson(talSyncUrl), "  url:", url)
				}
			}
...
func parseCerAndGetSubRepoUrlAndAddToSpQueue(spQueue *SyncParseQueue, cerFiles []string) {
	...
	for _, cerFile := range cerFiles {
		...
		if len(subRepoUrl) > 0 {
			...
			// no error
			belogs.Debug("parseCerAndGetSubRepoUrlAndAddToSpQueue(): this file has no subRepoUrl:", cerFile)
		}
	}
...
func parseCerAndGetSubRepoUrl(spQueue *SyncParseQueue, cerFile string) (subRepoUrl string, err error) {
	...
	if len(subRepoUrl) == 0 {
		belogs.Debug("parseCerAndGetSubRepoUrl(): all rsyncUrl or rrdpUrl is empty:", cerFile, jsonutil.MarshalJson(parseCerSimple))
