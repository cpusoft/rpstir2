...
func syncToLocalAndParseValidateCer(tmpDir, talUrl, subjectPublicKeyInfo string, talSyncUrl *model.TalSyncUrl, wg *sync.WaitGroup) {
	...
	belogs.Debug("syncToLocalAndParseValidateCer(): externalVerifyHttps:", externalVerifyHttps)
	// if url is https, then get cer file by http; if url is rsync, then get cer file by rsnc
	if strings.HasPrefix(talUrl, "https:") {
		// should verify https
		resp, body, err := httpclient.GetHttpsWithConfig(talUrl,
			httpclient.NewHttpClientConfigWithParam(5, 3, "all", externalVerifyHttps))
		if err != nil {
		...
		if resp.StatusCode != http.StatusOK {
			belogs.Error("syncToLocalAndParseValidateCer(): GetHttpsVerify StatusCode != http.StatusOK :", talUrl, resp.StatusCode)
			talSyncUrl.Error = "http status code is not http.StatusOK"
			talSyncUrl.SupportRrdp = false
			return
		}
		// save to localFile
		...
		// rsycn to local file
		rsyncTimeoutStr := conf.String("rsync::rsyncTimeout")
		rsyncConTimeoutStr := conf.String("rsync::rsyncConTimeout")
		rsyncDestPath, _, err := rsyncutil.RsyncQuietWithConfig(talUrl, tmpDir,
			rsyncutil.NewRsyncClientConfig(rsyncTimeoutStr, rsyncConTimeoutStr))
		if err != nil {
...
func parseAndValidateCer(talUrl, subjectPublicKeyInfo, tmpDir string, talSyncUrl *model.TalSyncUrl) (err error) {
	...
	belogs.Info("parseAndValidateCer(): PostFileAndUnmarshalResponseModel file :", talSyncUrl.LocalFile,
	...
	if conf.String("sync::supportTestCer") == "true" {
		if strings.Contains(talUrl, "zdns.cn") && strings.Contains(talSyncUrl.LocalFile, "zdns-test.cer") {
			...
			belogs.Info("parseAndValidateCer():supportTestCer add RpkiNotify, talUrl, talSyncUrl.LocalFile:", talUrl, talSyncUrl.LocalFile,
				"    parseCerSimple:", jsonutil.MarshalJson(parseCerSimple))
		}
		if strings.Contains(talUrl, "fzca.com") && len(parseCerSimple.RpkiNotify) == 0 {
			parseCerSimple.RpkiNotify = "https://rpki.fzca.com/rrdp/notification.xml"
			//parseCerSimple.CaRepository = ""
			belogs.Info("parseAndValidateCer():supportTestCer, add RpkiNotify, parseCerSimple:", jsonutil.MarshalJson(parseCerSimple))
		}
	}
	...
	if len(parseCerSimple.RpkiNotify) > 0 {
		start := time.Now()
		timeoutMins := conf.Int("rrdp::rrdpNoticationTimeoutSeconds") / 60
		externalVerifyHttps := conf.Bool("sync::externalVerifyHttps")
		belogs.Debug("parseAndValidateCer(): test rrdp is ok:", parseCerSimple.RpkiNotify, " timeoutMins:", timeoutMins,
			"  externalVerifyHttps:", externalVerifyHttps)
		httpConfig := httpclient.NewHttpClientConfigWithParam(uint64(timeoutMins), 3, "all", externalVerifyHttps)
		_, err = rrdputil.GetRrdpNotificationWithConfig(parseCerSimple.RpkiNotify, httpConfig)
		if err != nil {
			belogs.Error("parseAndValidateCer(): rrdputil.GetRrdpNotificationWithConfig fail:", parseCerSimple.RpkiNotify,
	...
	if len(parseCerSimple.CaRepository) > 0 {
		...
		if strings.HasPrefix(talSyncUrl.TalUrl, "rsync:") {
			...
			rsyncTimeoutStr := conf.String("rsync::rsyncTimeout")
			rsyncConTimeoutStr := conf.String("rsync::rsyncConTimeout")
			_, _, err := rsyncutil.RsyncQuietWithConfig(talUrl, tmpDir,
				rsyncutil.NewRsyncClientConfig(rsyncTimeoutStr, rsyncConTimeoutStr))
			if err != nil {
