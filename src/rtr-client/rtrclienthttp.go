...
func ClientStart(c *gin.Context) {
	...
	err := c.ShouldBindJSON(&rtrClientStartModel)
	if err != nil {
		belogs.Error("ServerSendSerialNotify(): ShouldBindJSON get rtrClientStartModel fail, err:", err)
		ginserver.ResponseFail(c, err, "")
		return
	}
	belogs.Debug("ClientStart(): rtrClientStartModel:", jsonutil.MarshalJson(rtrClientStartModel))
...
func ClientSendSerialQuery(c *gin.Context) {
	...
	if err != nil {
		belogs.Error("ClientSendSerialQuery(): ShouldBindJSON get rtrClientSerialQueryModel fail:", err)
		ginserver.ResponseFail(c, err, "")
		return
	}
