...
func ParseEeCertModel(eeCertFile string, fileByte []byte, start int, end int) (eeCertModel model.EeCertModel, err error) {
...
func ValidateEeCertModel(stateModel *model.StateModel, eeCertModel *model.EeCertModel) error {
	...
	if eeCertModel.DigestAlgorithm != "SHA256-RSA" {
		belogs.Info("ValidateEeCertModel(): check eeCertModel.DigestAlgorithm fail, eeCertModel.DigestAlgorithm:", jsonutil.MarshalJson(eeCertModel.DigestAlgorithm))
		stateMsg := model.StateMsg{Stage: "parsevalidate",
