...
func delCerDb(session *xorm.Session, filePathPrefix string) (err error) {
	start := time.Now()
	belogs.Debug("delCerDb():will delete lab_rpki_cer by filePathPrefix:", filePathPrefix)
	...
	cerIdsStr := stringutil.Int64sToInString(cerIds)
	belogs.Debug("delCerDb():will delete lab_rpki_cer len(cerIds):", len(cerIds), "   filePathPrefix:", filePathPrefix)
	...
	// del ipIds
	ipIdsStr := stringutil.Int64sToInString(ipIds)
	if len(ipIdsStr) > 0 {
	...
	// del crldpIds
	crldpIdsStr := stringutil.Int64sToInString(crldpIds)
	...
	// del asnIds
	asnIdsStr := stringutil.Int64sToInString(asnIds)
	if len(asnIdsStr) > 0 {
	...
	belogs.Info("delCerDb():delete lab_rpki_cer  ok, by filePathPrefix:", filePathPrefix,
