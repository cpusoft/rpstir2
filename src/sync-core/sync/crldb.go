...
func delCrlDb(session *xorm.Session, filePathPrefix string) (err error) {
	start := time.Now()
	belogs.Debug("delCrlDb():will delete lab_rpki_crl by filePathPrefix:", filePathPrefix)
	...
	crlIdsStr := stringutil.Int64sToInString(crlIds)
	belogs.Debug("delCrlDb():will delete lab_rpki_crl len(crlIds):", len(crlIds), "   filePathPrefix:", filePathPrefix)
	...
	// del revokeIds
	revokeIdsStr := stringutil.Int64sToInString(revokeIds)
	...
	belogs.Info("delCrlDb():delete lab_rpki_crl ok, by filePathPrefix:", filePathPrefix,
