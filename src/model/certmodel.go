...
func (c CerModel) String() string {
	...
	m["len(cerIpAddresses)"] = len(c.CerIpAddressModel.CerIpAddresses)
	m["len(asns)"] = len(c.AsnModel.Asns)
...
func (c AsnModel) String() string {
	return jsonutil.MarshalJson(c.Asns)
}
...
func (c CrlModel) String() string {
	m := make(map[string]interface{})
	m["thisUpdate"] = c.ThisUpdate
	m["nextUpdate"] = c.NextUpdate
	m["aki"] = c.Aki
	m["crlNumber"] = c.CrlNumber
	m["filePath"] = c.FilePath
	m["fileName"] = c.FileName
	m["len(revokedCertModels)"] = len(c.RevokedCertModels)
	return jsonutil.MarshalJson(m)
}
...
func (c MftModel) String() string {
	m := make(map[string]interface{})
	m["thisUpdate"] = c.ThisUpdate
	m["nextUpdate"] = c.NextUpdate
	m["ski"] = c.Ski
	m["aki"] = c.Aki
	m["mftNumber"] = c.MftNumber
	m["filePath"] = c.FilePath
	m["fileName"] = c.FileName
	m["aiaModel"] = c.AiaModel.String()
	m["siaModel"] = c.SiaModel.String()
	m["len(fileHashModels)"] = len(c.FileHashModels)
	return jsonutil.MarshalJson(m)
}
...
func ConvertCertModelToEeCertModel(cerModel *CerModel,
	eeCertStart, eeCertEnd uint64) *EeCertModel {
	...
	return &EeCertModel{
		...
		KeyUsageModel:   cerModel.KeyUsageModel,
		ExtKeyUsages:    cerModel.ExtKeyUsages,

		BasicConstraintsValid: cerModel.BasicConstraintsModel.BasicConstraintsValid,
		IsCa:                  cerModel.IsCa,

		SubjectAll: cerModel.SubjectAll,
		IssuerAll:  cerModel.IssuerAll,

		SiaModel: cerModel.SiaModel,
		// in roa, ee cert also has ip address
		CerIpAddressModel: cerModel.CerIpAddressModel,

		CrldpModel: cerModel.CrldpModel,

		// in mft/roa , eecert start/end byte:
		// eeCertByte:=OraByte[EeCertStart:EeCertEnd] eeCertByte:=MftByte[EeCertStart:EeCertEnd]
		EeCertStart: eeCertStart,
		EeCertEnd:   eeCertEnd,
	}
}
...
func (c SigModel) String() string {
	...
	m["fileName"] = c.FileName
	m["aiaModel"] = c.AiaModel.String()
	m["len(cerIpAddresses)"] = len(c.RpkiSignedChecklist.CerIpAddresses)
	m["len(fileHashModels)"] = len(c.RpkiSignedChecklist.FileHashModels)
	return jsonutil.MarshalJson(m)
}
...
func (c AsaModel) String() string {
	...
	m["aiaModel"] = c.AiaModel.String()
	m["siaModel"] = c.SiaModel.String()
	m["len(customerAsns)"] = len(c.CustomerAsns)
	return jsonutil.MarshalJson(m)
}
