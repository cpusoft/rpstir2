...
type DistributedSnapshotModel struct {
	...
	Rir         string `json:"rir" xorm:"rir varchar(512)"`
	...
	CenterRrdpDestPath string `json:"centerRrdpDestPath"`
	Index              uint64 `json:"index"`
	SyncLogId          uint64 `json:"syncLogId"`
	NodeUrl            string `json:"nodeUrl"`

	SnapshotConcurrentCount uint64 `json:"snapshotConcurrentCount"`
}
...
func NewDistributedSnapshotCountResult(distributedSnapshotModel *DistributedSnapshotModel,
	...
	return &DistributedSnapshotCountResult{
		...
		SyncTime:     syncTime,
		NodeUrl:      distributedSnapshotModel.NodeUrl,
	}
}
...
type DistributedDeltaModel struct {
	...
	DeltaConcurrentCount uint64 `json:"deltaConcurrentCount"`
}
...
func NewDistributedDeltaCountResult(distributedDeltaModel *DistributedDeltaModel,
	...
	return &DistributedDeltaCountResult{
		...
		SyncTime:      syncTime,
		NodeUrl:       distributedDeltaModel.NodeUrl,
	}
}
...
type DistributedPublishWithdrawResult struct {
	...
	CenterNodeUrl        string `json:"centerNodeUrl"`
	//
	CenterFilePathName string      `json:"centerFilePathName"`
...
func (c DistributedPublishWithdrawResult) String() string {
	...
	m["centerFilePathName"] = c.CenterFilePathName
	m["len(fileHash)"] = len(c.FileHash)
	m["len(base64)"] = len(c.Base64)
...
func NewDistributedPublishWithdrawResultFromSnapshot(distributedSnapshotModel *DistributedSnapshotModel,
	...
	destPath := "" // in distributed node, not include conf.String("rrdp::destPath") + "/""
	filePathName, _ := urlutil.JoinPrefixPathAndUrlFileName(destPath, snapshotPublish.Uri)
	c.CenterFilePathName = osutil.JoinPathFile(distributedSnapshotModel.CenterRrdpDestPath, filePathName)
...
func NewDistributedPublishWithdrawResultFromDelta(distributedDeltaModel *DistributedDeltaModel,
	...
	destPath := "" // in distributed node, not include conf.String("rrdp::destPath") + "/""
	filePathName, _ := urlutil.JoinPrefixPathAndUrlFileName(destPath, c.PublishOrWithdrawUrl)
	c.CenterFilePathName = osutil.JoinPathFile(distributedDeltaModel.CenterRrdpDestPath, filePathName)
