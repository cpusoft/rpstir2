...
func saveToSyncRrdpNotifyDb(distributedResult *model.DistributedResult) error {
	...
	if err != nil {
		belogs.Error("saveToSyncRrdpNotifyDb(): update sync_rrdp_notify fail, curSerial:",
			curSerial, "  notifyUrl:", notifyUrl, "  downloadTime:", downloadTime, err)
		return xormdb.RollbackAndLogError(session, "saveToSyncRrdpNotifyDb(): update sync_rrdp_notify fail:"+
			notifyUrl, err)
	}
	rows, err := affected.RowsAffected()
	belogs.Debug("saveToSyncRrdpNotifyDb(): update sync_rrdp_notify, notifyUrl:", notifyUrl, "  affected rows:", rows)
